...
<body>
    ...
    <main>
      <div class="topbar">
        ...
        <div class="filters">
          ...
          <button id="importReview">Import JSON</button>
          <input id="importReviewFile" type="file" accept=".json,application/json" hidden>
  ...
  <script>
    ...
    async function loadDefaultReviewState() {{
      if (hasMeaningfulState(state)) return;
      try {{
        const response = await fetch("review_state.json", {{ cache: "no-store" }});
        if (!response.ok) return;
        const imported = normalizeImportedReview(await response.json());
        if (!Object.keys(imported).length) return;
        state = imported;
        save();
      }} catch (error) {{
        console.info("No default review state loaded.", error);
      }}
    }}
    ...
    function normalizeImportedReview(data) {{
      const rows = Array.isArray(data) ? data : Object.entries(data || {{}}).map(([id, review]) => ({{ id, ...review }}));
      ...
      rows.forEach(row => {{
        ...
        imported[id] = {{
          verdict: ["pick", "hold", "reject"].includes(row.verdict) ? row.verdict : "",
    ...
    async function importReviewRows(file) {{
      try {{
        const text = await file.text();
        const imported = normalizeImportedReview(JSON.parse(text));
        const count = Object.keys(imported).length;
        if (!count) {{
          alert("가져올 리뷰 데이터가 없습니다.");
          return;
        }}
        state = {{ ...state, ...imported }};
        save();
        render();
        const pickCount = Object.values(imported).filter(row => row.verdict === "pick").length;
        alert(`${{count}}개 리뷰 상태를 가져왔습니다. Pick ${{pickCount}}개가 반영되었습니다.`);
      }} catch (error) {{
        console.error(error);
        alert("JSON을 읽지 못했습니다. JSON Export 또는 Picked JSON 파일을 선택해주세요.");
      }}
    }}
    ...
    document.getElementById("importReview").onclick = () => document.getElementById("importReviewFile").click();
    document.getElementById("importReviewFile").onchange = e => {{
      const file = e.target.files?.[0];
      if (file) importReviewRows(file);
      e.target.value = "";
    }};
    ...
    loadDefaultReviewState().finally(render);
  </script>
</body>
</html>
"""
